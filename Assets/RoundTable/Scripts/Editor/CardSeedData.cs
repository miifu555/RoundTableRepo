...
namespace RoundTable.EditorTools
{
    ...
    public static class CardSeedData
    {
        ...
        static void Build()
        {
            ...
            Atk("a3", "情報収集", 3, 2, "2枚カードを引く", E(EffectKind.DrawCards, 2));
            Atk("a4", "連休の始まり", 7, 1, "このターンのあと、エクストラターンを得る", E(EffectKind.ExtraTurn));
            Fld("f1", "一息つく", 3, 4, "番の終わり、気力を2回復する", TriggerKind.OnOwnTurnEnd, E(EffectKind.Heal, 2));
            Fld("f2", "鉄壁の言い訳", 3, 3, "削られる気力を-1する", TriggerKind.None, E(EffectKind.PassiveDamageReduction, 1));

            // ===== 鈴木　アグロ(運メイン) / シャッフルタイプ =====
            Deck("suzuki", "鈴木", "アグロ(運メイン)", DrawStyle.Shuffle);
            Atk("a1", "軽い挑発", 2, 2, "相手の気力を2削る", E(EffectKind.Damage, 2));
            Atk("a2", "出たとこ勝負", 2, 4, "4面サイコロで出た目分気力を削る", E(EffectKind.DamageDice, diceSides: 4));
            Atk("a3", "山勘", 3, 2, "2枚カードを引く", E(EffectKind.DrawCards, 2));
            Atk("a4", "無差別掃除", 1, 2, "4面サイコロで出た目-2個相手のフィールドを破壊する", E(EffectKind.DestroyOpponentFieldDice, diceSides: 4, modifier: -2));
            Atk("a5", "気まぐれ探索", 1, 3, "4面サイコロで出た目-2枚カードを引く", E(EffectKind.DrawCardsDice, diceSides: 4, modifier: -2));
            ...
            Atk("a1", "小言", 2, 3, "相手の気力を2削る", E(EffectKind.Damage, 2));
            Atk("a2", "下調べ", 1, 3, "1枚カードを引く", E(EffectKind.DrawCards, 1));
            Atk("a3", "計画的撤去", 3, 1, "相手のフィールドを一つ破壊する", E(EffectKind.DestroyOpponentField, 1));
            Fld("f1", "意趣返し", 3, 2, "気力を削られた時、相手の気力も1削る", TriggerKind.OnDamaged, E(EffectKind.Damage, 1));
            Fld("f2", "連帯責任", 3, 4, "気力を削られた時、相手のフィールドを一つ破壊する", TriggerKind.OnDamaged, E(EffectKind.DestroyOpponentField, 1));
            ...
            Atk("a1", "正論パンチ", 4, 2, "相手の気力を3削る", E(EffectKind.Damage, 3));
            Atk("a2", "爆弾発言", 3, 4, "相手の山札を5枚削る", E(EffectKind.MillOpponentDeck, 5));
            Atk("a3", "ネタ探し", 2, 3, "3枚カードを引く", E(EffectKind.DrawCards, 3));
            Atk("a4", "話題の切り替え", 1, 2, "4面サイコロで出た目-2個相手のフィールドを破壊する", E(EffectKind.DestroyOpponentFieldDice, diceSides: 4, modifier: -2));
            Fld("f1", "削れる語彙", 2, 4, "気力を削られた時、相手の山札を上から1枚削る", TriggerKind.OnDamaged, E(EffectKind.MillOpponentDeck, 1));

            // ===== かっしー　アグロ(一点集中) / ドロータイプ =====
            Deck("kasshi", "かっしー", "アグロ(一点集中)", DrawStyle.Draw);
            Atk("a1", "一点突破", 3, 3, "相手の気力を4削る", E(EffectKind.Damage, 4));
            Atk("a2", "渾身の一撃", 6, 3, "相手の気力を6削る", E(EffectKind.Damage, 6));
            Atk("a3", "深呼吸", 3, 1, "カードを3枚引く", E(EffectKind.DrawCards, 3));
            Atk("a4", "なぎ払い", 5, 3, "相手のフィールドを2個破壊する", E(EffectKind.DestroyOpponentField, 2));
            Atk("a5", "大振り", 4, 2, "6面サイコロで出た目分気力を削る", E(EffectKind.DamageDice, diceSides: 6));
            Fld("f1", "反抗心", 3, 3, "気力を削られた時、カードを1枚ドローする", TriggerKind.OnDamaged, E(EffectKind.DrawCards, 1));
            ...
            Fld("f2", "古武術", 2, 4, "自分のターン開始時、相手の気力を1削る", TriggerKind.OnOwnTurnStart, E(EffectKind.Damage, 1));

            // ===== すどー　アグロ(除去) / シャッフルタイプ =====
            Deck("sudo", "すどー", "アグロ(除去)", DrawStyle.Shuffle);
            ...
            Atk("a1", "不意打ち", 1, 3, "相手の気力を3削る", E(EffectKind.Damage, 3));
            Atk("a2", "情報屋", 1, 3, "2枚カードを引く", E(EffectKind.DrawCards, 2));
            Atk("a3", "抜き取り", 1, 1, "相手の手札を2枚トラッシュする", E(EffectKind.DiscardOpponentHand, 2));
            Atk("a4", "証拠隠滅", 2, 2, "相手のフィールドを１つ破壊する", E(EffectKind.DestroyOpponentField, 1));
            Fld("f1", "取り立て", 2, 6, "カードで気力を削られたとき、相手の手札をランダムに1枚トラッシュする", TriggerKind.OnDamagedByCard, E(EffectKind.DiscardOpponentHand, 1));

            // ===== こうの　アグロ(必殺) / ドロータイプ =====
            Deck("kono", "こうの", "アグロ(必殺)", DrawStyle.Draw);
